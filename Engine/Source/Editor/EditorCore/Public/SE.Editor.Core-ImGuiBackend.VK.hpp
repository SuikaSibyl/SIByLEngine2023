...
SE_EXPORT struct ImGuiTexture_VK : public ImGuiTexture {
  /** initialzie */
  ImGuiTexture_VK(RHI::Sampler* sampler, RHI::TextureView* view,
...
ImGuiTexture_VK::ImGuiTexture_VK(RHI::Sampler* sampler, RHI::TextureView* view,
                                 RHI::TextureLayout layout) {
...
SE_EXPORT struct ImGuiBackend_VK : public ImGuiBackend {
  /** initializer */
  ...
  virtual auto createImGuiTexture(RHI::Sampler* sampler, RHI::TextureView* view,
      ...
      -> std::unique_ptr<ImGuiTexture> override;
...
ImGuiBackend_VK::ImGuiBackend_VK(RHI::RHILayer* rhiLayer)
  ...
  mainWindowData.Surface =
      static_cast<RHI::Context_VK*>(rhiLayer->getContext())->getVkSurfaceKHR();
  ...
  mainWindowData.SurfaceFormat = ImGui_ImplVulkanH_SelectSurfaceFormat(
      static_cast<RHI::Adapter_VK*>(rhiLayer->getAdapter())
          ->getVkPhysicalDevice(),
      mainWindowData.Surface, requestSurfaceImageFormat,
  ...
  mainWindowData.PresentMode = ImGui_ImplVulkanH_SelectPresentMode(
      static_cast<RHI::Adapter_VK*>(rhiLayer->getAdapter())
          ->getVkPhysicalDevice(),
      mainWindowData.Surface, &present_modes[0], IM_ARRAYSIZE(present_modes));
  ...
  int width, height;
  rhiLayer->getContext()->getBindedWindow()->getFramebufferSize(&width,
                                                                &height);
...
auto ImGuiBackend_VK::setupPlatformBackend() noexcept -> void {
  ...
  init_info.Queue =
      static_cast<RHI::Queue_VK*>(rhiLayer->getDevice()->getGraphicsQueue())
          ->queue;
...
void check_vk_result(VkResult err) {
  ...
  Core::LogManager::Error(
      std::format("ImGui Vulkan Error: VkResult = {0}", (unsigned int)err));
  if (err < 0) return;
}
...
auto ImGuiBackend_VK::render(ImDrawData* draw_data) -> void {
  ...
  {
    err = vkWaitForFences(
        static_cast<RHI::Device_VK*>(rhiLayer->getDevice())->getVkDevice(), 1,
        &fd->Fence, VK_TRUE,
...
auto ImGuiBackend_VK::createImGuiTexture(RHI::Sampler* sampler,
