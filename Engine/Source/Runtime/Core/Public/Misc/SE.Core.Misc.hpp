...
SE_EXPORT struct WorldTimePoint {
...
inline auto Timer::deltaTime() noexcept -> double { return _deltaTime; }

inline auto Timer::totalTime() noexcept -> double {
  uint64_t const totalTimeCount = uint64_t(
      std::chrono::duration<double, std::micro>(prevTimePoint - startTimePoint)
          .count());
