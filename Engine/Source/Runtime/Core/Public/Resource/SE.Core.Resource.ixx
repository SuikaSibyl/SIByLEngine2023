...
export module SE.Core.Resource;
import SE.Core.Log;
...
namespace SIByL::Core
{
	...
	export constexpr inline GUID INVALID_GUID = uint64_t(-1);
    /** Offline Resource ID */
    export using ORID = uint64_t;
    export constexpr inline ORID INVALID_ORID = uint64_t(-1);

	/** hash a filesystem path to a GUID */
    export inline auto hashUID(char const* path) noexcept -> uint64_t;
