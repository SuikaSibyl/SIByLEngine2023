...
struct vec3f {
  ...
  inline vec3f integer() {
    return vec3f(double(int(x)), double(int(y)), double(int(z)));
  }
  ...
  double random_double_01(double a) {
    double rnf = a * 14.434252 + a * 364.2343 + a * 4213.45352 +
                 a * 2341.43255 + a * 254341.43535 + a * 223454341.3523534245 +
                 23453.423412;
...
double calculate_error(int id_v1, int id_v2, vec3f &p_result) {
  ...
  if (det != 0 && !border) {
    ...
    p_result.x =
        -1 / det * (q.det(1, 2, 3, 4, 5, 6, 5, 7, 8));  // vx = A41/det(q_delta)
    p_result.y =
        1 / det * (q.det(0, 2, 3, 1, 5, 6, 2, 7, 8));  // vy = A42/det(q_delta)
    p_result.z =
        -1 / det * (q.det(0, 1, 3, 1, 4, 6, 2, 5, 8));  // vz = A43/det(q_delta)
...
void load_obj(const char *filename, bool process_uv = false) {
  ...
  if (process_uv && uvs.size()) {
    loopi(0, triangles.size()) {
      loopj(0, 3) triangles[i].uvs[j] = uvs[uvMap[i][j]];
    }
  }
...
void write_obj(const char *filename) {
  ...
  int cur_material = -1;
  bool has_uv =
      (triangles.size() && (triangles[0].attr & TEXCOORD) == TEXCOORD);
  ...
  loopi(0, triangles.size()) if (!triangles[i].deleted) {
    ...
    if (has_uv) {
      fprintf(file, "f %d/%d %d/%d %d/%d\n", triangles[i].v[0] + 1, uv,
              triangles[i].v[1] + 1, uv + 1, triangles[i].v[2] + 1, uv + 2);
      uv += 3;
    } else {
    ...
    // fprintf(file, "f %d// %d// %d//\n", triangles[i].v[0]+1,
    // triangles[i].v[1]+1, triangles[i].v[2]+1); //more compact: remove
    // trailing zeros
  }
  fclose(file);
}
