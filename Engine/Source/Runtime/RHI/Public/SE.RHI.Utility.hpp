...
namespace SIByL::RHI {
SE_EXPORT struct ResourceBookKeeper {
  ...
  auto generateBarrierDescriptors() noexcept -> std::vector<BarrierDescriptor> {
    ...
    std::unordered_map<PipelineStageFlags, BarrierDescriptor>
        barrierDescriptorMap;
    for (auto bufferConsumePair : bufferBookKeeping) {
      ...
      if (bufferConsumePair.second[1].buffer == nullptr) continue;
      auto descriptorMapIter = barrierDescriptorMap.find(
          bufferConsumePair.second[0].pipelineStageFlags);
      if (descriptorMapIter == barrierDescriptorMap.end()) {
        barrierDescriptorMap[bufferConsumePair.second[0].pipelineStageFlags] =
            BarrierDescriptor{
                ...
                {},
                {}};
        descriptorMapIter = barrierDescriptorMap.find(
            bufferConsumePair.second[0].pipelineStageFlags);
      }
      BarrierDescriptor& barrierDesc = descriptorMapIter->second;
      barrierDesc.bufferMemoryBarriers.emplace_back(
    ...
    for (auto textureConsumePair : textureBookKeeping) {
      ...
      // not consumed in this pipeline, so just omit it.
      if (textureConsumePair.second[1].texture == nullptr) continue;
      auto descriptorMapIter = barrierDescriptorMap.find(
          textureConsumePair.second[0].pipelineStageFlags);
      if (descriptorMapIter == barrierDescriptorMap.end()) {
        barrierDescriptorMap[textureConsumePair.second[0].pipelineStageFlags] =
            BarrierDescriptor{
      ...
      BarrierDescriptor& barrierDesc = descriptorMapIter->second;
      barrierDesc.textureMemoryBarriers.emplace_back(
          TextureMemoryBarrierDescriptor{
              textureConsumePair.second[1].texture,  // texture
              textureConsumePair.second[1]
                  .subresourceRange,  // TODO: different subresource
    ...
    std::vector<BarrierDescriptor> barrierDescriptors(
        barrierDescriptorMap.size());
    for (auto& pair : barrierDescriptorMap)
      barrierDescriptors.emplace_back(pair.second);
    return barrierDescriptors;
  }

  std::unordered_map<Buffer*, std::array<BufferConsumeDescriptor, 2>>
      bufferBookKeeping;
  std::unordered_map<Texture*, std::array<TextureConsumeDescriptor, 2>>
      textureBookKeeping;
};
}  // namespace SIByL::RHI