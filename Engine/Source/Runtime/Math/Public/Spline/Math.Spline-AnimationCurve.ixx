...
#include <vector>
#include <algorithm>
export module Math.Spline: AnimationCurve;
...
namespace SIByL::Math
{
	...
	public:
		...
		AnimationCurve(std::initializer_list<KeyFrame> const& initializer_list) :keyFrames(initializer_list) { sortAllKeyFrames(); }
	...
	auto AnimationCurve::evaluate(float time) noexcept -> float {
		if (time > keyFrames.back().time) { // right warp
			...
			{
				int passCount = int((time - keyFrames.back().time) / (keyFrames.back().time - keyFrames.front().time));
				time = time - (passCount + 1) * (keyFrames.back().time - keyFrames.front().time);
			}
			break;
			case WrapMode::PINGPOMG:
			{
				...
				time = time - (passCount + 1) * (keyFrames.back().time - keyFrames.front().time);
				if (needReverse) time = keyFrames.front().time + keyFrames.back().time - time;
			}
			break;
			default:
				break;
			...
			case WrapMode::CLAMP:
				...
				break;
			case WrapMode::REPEAT: {
				int passCount = int((keyFrames.front().time - time) / (keyFrames.back().time - keyFrames.front().time));
				time = time + (passCount + 1) * (keyFrames.back().time - keyFrames.front().time);
			}
			break;
			case WrapMode::PINGPOMG: {
		...
		while (left + 1 < keyFrames.size()) {
			if (keyFrames[left].time <= time && keyFrames[left + 1].time > time)
				break;
			left++;
		}
	...
	auto compareKeyFrameByTime(KeyFrame const& lhv, KeyFrame const& rhv) noexcept -> bool {
