...
#include <cstdlib>
#include <cmath>
export module SE.Math.Geometric:Vector3;
import :Vector2;
...
namespace SIByL::Math
{
	...
	export template <class T> inline auto floor(Vector3<T> const& v) noexcept -> Vector3<T>
	{
		Vector3<T> result;
		for (size_t i = 0; i < 3; ++i) {
	...
	export template <class T> inline auto ceil(Vector3<T> const& v) noexcept -> Vector3<T>
	{
		Vector3<T> result;
		for (size_t i = 0; i < 3; ++i) {
	...
	export template <class T> inline auto length(Vector3<T> const& x) noexcept -> T {
