...
export module SE.SRenderer.BarLCPass;
...
namespace SIByL
{
	export struct BarLCPass :public RDG::FullScreenPass {

		BarLCPass() {
			frag = GFX::GFXManager::get()->registerShaderModuleResource(
				"../Engine/Binaries/Runtime/spirv/SRenderer/rasterizer/ssrt/ssrt_lc_debugger_frag.spv",
				{ nullptr, RHI::ShaderStages::FRAGMENT });
			RDG::FullScreenPass::init(Core::ResourceManager::get()->getResource<GFX::ShaderModule>(frag));

			uniform_buffer = GFX::GFXManager::get()->createStructuredUniformBuffer<Uniform>();
		}
		...
		virtual auto renderUI() noexcept -> void override {
			{	// Select an item type
				const char* item_names[] = {
					"DI", "Specular", "Diffuse", "Debug Specular Ray", "Debug Occlusion Ray",
					"Show Normal Cone", "Show Tex Jacobian", "Visualize Importance"
				};
