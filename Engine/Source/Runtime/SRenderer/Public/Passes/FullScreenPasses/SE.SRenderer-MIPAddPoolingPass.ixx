...
namespace SIByL
{
	export struct MIPAddPoolingSubPass :public RDG::FullScreenPass {

		MIPAddPoolingSubPass(size_t mip_offset) :mipOffset(mip_offset) {
			frag = GFX::GFXManager::get()->registerShaderModuleResource(
				"../Engine/Binaries/Runtime/spirv/SRenderer/rasterizer/fullscreen_pass/sumup_mip_float32_frag.spv",
	...
	export struct MIPAddPoolingPass :public RDG::Subgraph {
		...
		virtual auto alias() noexcept -> RDG::AliasDict override {
			...
			dict.addAlias("Output", CONCAT("Pre-Z Pass " + std::to_string(mipCount - 2)), "R32");
			return dict;
		}

		virtual auto onRegister(RDG::Graph* graph) noexcept -> void override {
			for (size_t i = 0; i < mipCount - 1; ++i) {
				graph->addPass(std::make_unique<MIPAddPoolingSubPass>(i), CONCAT("Pre-Z Pass " + std::to_string(i)));
				if (i != 0) graph->addEdge(CONCAT("Pre-Z Pass " + std::to_string(i - 1)), "R32", CONCAT("Pre-Z Pass " + std::to_string(i)), "R32");
			}
		}
