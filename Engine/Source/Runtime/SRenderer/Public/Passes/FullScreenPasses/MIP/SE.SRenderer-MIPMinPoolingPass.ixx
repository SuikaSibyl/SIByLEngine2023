...
export module SE.SRenderer.MIPMinPoolingPass;
...
namespace SIByL
{
	export struct MIPMinPoolingSubPass :public RDG::FullScreenPass {

		MIPMinPoolingSubPass(size_t mip_offset) :mipOffset(mip_offset) {
			frag = GFX::GFXManager::get()->registerShaderModuleResource(
				"../Engine/Binaries/Runtime/spirv/SRenderer/rasterizer/fullscreen_pass/min_mip_float32_frag.spv",
		...
		virtual auto execute(RDG::RenderContext* context, RDG::RenderData const& renderData) noexcept -> void {
			...
			renderPassDescriptor = {
				{ RHI::RenderPassColorAttachment{target->getRTV(mipOffset + 1, 0, 1), nullptr, {0,0,0,1}, RHI::LoadOp::CLEAR, RHI::StoreOp::STORE }},
	...
	export struct MIPMinPoolingPass :public RDG::Subgraph {

		MIPMinPoolingPass(size_t width, size_t height) :width(width), height(height) {
		...
		virtual auto alias() noexcept -> RDG::AliasDict override {
			...
			dict.addAlias("Output", CONCAT("MinPool Pass " + std::to_string(mipCount - 2)), "R32");
			//dict.addAlias("Input", CONCAT("Input Dummy"), "F32");
		...
		virtual auto onRegister(RDG::Graph* graph) noexcept -> void override {
			...
			graph->addPass(std::make_unique<BlitPass>(BlitPass::Descriptor{ 0,0,0,0 }), CONCAT("Blit Pass"));
			graph->addPass(std::make_unique<MIPMinPoolingSubPass>(0), CONCAT("MinPool Pass " + std::to_string(0)));

			graph->addEdge(CONCAT("Input Dummy"), "F32", CONCAT("Blit Pass"), "Source");
			graph->addEdge(CONCAT("Input Dummy"), "F32MIP", CONCAT("Blit Pass"), "Target");
			graph->addEdge(CONCAT("Blit Pass"), "Target", CONCAT("MinPool Pass 0"), "R32");

			for (size_t i = 1; i < mipCount - 1; ++i) {
				graph->addPass(std::make_unique<MIPMinPoolingSubPass>(i), CONCAT("MinPool Pass " + std::to_string(i)));
				graph->addEdge(CONCAT("MinPool Pass " + std::to_string(i - 1)), "R32", CONCAT("MinPool Pass " + std::to_string(i)), "R32");
			}
		}
