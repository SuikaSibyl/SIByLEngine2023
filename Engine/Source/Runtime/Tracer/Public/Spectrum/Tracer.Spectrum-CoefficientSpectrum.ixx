...
namespace SIByL::Tracer
{
	...
	*/
	...
	public:
		...
		auto operator-()const->CoefficientSpectrum;
		...
		auto clamp(float low = 0.f, float high = FLT_MAX) noexcept -> CoefficientSpectrum;

		friend inline auto sqrt(CoefficientSpectrum const& s) noexcept -> CoefficientSpectrum;
		friend inline auto exp(CoefficientSpectrum const& s) noexcept -> CoefficientSpectrum;
		...
		auto operator[](size_t i) const -> float { return c[i]; }
	...
	*/
	...
	template<size_t nSpectrumSamples>
	auto CoefficientSpectrum<nSpectrumSamples>::operator-()const->CoefficientSpectrum
	{
		CoefficientSpectrum<nSpectrumSamples> res;
		for (size_t i = 0; i < nSpectrumSamples; ++i)
			res.c[i] = -c[i];
		return res;
	}
	...
	*/
	...
	export template<size_t nSpectrumSamples>
		inline auto exp(CoefficientSpectrum<nSpectrumSamples> const& s) noexcept -> CoefficientSpectrum<nSpectrumSamples>
	{
		CoefficientSpectrum<nSpectrumSamples> ret;
		for (size_t i = 0; i < nSpectrumSamples; ++i)
