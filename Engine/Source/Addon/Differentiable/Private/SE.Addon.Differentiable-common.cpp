...
auto LossReadbackTexPass::execute(
  ...
  Math::ivec2 const size = {int(loss_tex->texture->width()), int(loss_tex->texture->height())};
  RHI::ComputePassEncoder* encoder = beginPass(context);
  encoder->pushConstants((void*)&size, (uint32_t)RHI::ShaderStages::COMPUTE, 0, sizeof(size));
  encoder->dispatchWorkgroups((size.x + 15) / 16, (size.y + 15) / 16);
  encoder->end();
}
...
auto MLPLayer::initialize_primal(std::span<float> primal,
    ...
    LinearLayer linear(16);
    for (int i = 0; i < N; ++i) linear.initialize_primal(
        primal.subspan(i * (C * C + C), (C * C + C)), initializer);
}

// FeatureGrid2D layer definition
FeatureGrid2D::FeatureGrid2D(Math::ivec2 grid, int dim) : grid(grid), dim(dim) {}
auto FeatureGrid2D::get_buffer_param_count() noexcept -> uint32_t { return grid.x*grid.y*dim; }
