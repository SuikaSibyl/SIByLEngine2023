...
SSPGGMM_ClearPass::SSPGGMM_ClearPass() {
  auto [comp] = GFX::ShaderLoader_SLANG::load(
      "../Engine/Shaders/SRenderer/addon/pathguiding/sspg-gmm-clear.slang",
      std::array<std::pair<std::string, RHI::ShaderStages>, 1>{
          std::make_pair("ComputeMain", RHI::ShaderStages::COMPUTE),
      });
...
auto SSPGGMM_ClearPass::reflect() noexcept -> RDG::PassReflection {
  ...
  reflector.addOutput("GMMStatisticsPack0")
      .isBuffer().withSize(4 * 1280 * 720 * sizeof(float) * 4)
      .withUsages((uint32_t)RHI::BufferUsage::STORAGE)
      .consume(RDG::BufferInfo::ConsumeEntry{}
...
auto SSPGGMM_ClearPass::execute(
    ...
    RDG::RenderData const& renderData) noexcept
    -> void {
  GFX::Buffer* pack0 = renderData.getBuffer("GMMStatisticsPack0");
  GFX::Buffer* pack1 = renderData.getBuffer("GMMStatisticsPack1");
  GFX::Buffer* pack0prev = renderData.getBuffer("GMMStatisticsPack0Prev");
  GFX::Buffer* pack1prev = renderData.getBuffer("GMMStatisticsPack1Prev");
  updateBindings(context, {
    {"u_gmmStatisticsPack0", RHI::BindingResource{{pack0->buffer.get(), 0, pack0->buffer->size()}}},
    {"u_gmmStatisticsPack1", RHI::BindingResource{{pack1->buffer.get(), 0, pack1->buffer->size()}}},
    {"u_gmmStatisticsPack0Prev", RHI::BindingResource{{pack0prev->buffer.get(), 0, pack0prev->buffer->size()}}},
    {"u_gmmStatisticsPack1Prev", RHI::BindingResource{{pack1prev->buffer.get(), 0, pack1prev->buffer->size()}}},});
...
SSPGGMM_SamplePass::SSPGGMM_SamplePass() {
  auto [rgen] = GFX::ShaderLoader_SLANG::load<1u>(
      "../Engine/Shaders/SRenderer/addon/"
      "pathguiding/sspg-gmm-sample.slang",
      std::array<std::pair<std::string, RHI::ShaderStages>, 1>{
          std::make_pair("RgenMain", RHI::ShaderStages::RAYGEN),
      });
...
auto SSPGGMM_LearnPass::reflect() noexcept -> RDG::PassReflection {
  ...
  reflector.addInputOutput("GMMStatisticsPack0")
      .isBuffer().withUsages((uint32_t)RHI::BufferUsage::STORAGE)
      .consume(RDG::BufferInfo::ConsumeEntry{}.setAccess(
              (uint32_t)RHI::AccessFlagBits::SHADER_WRITE_BIT |
...
auto SSPGGMM_LearnPass::execute(RDG::RenderContext* context,
  ...
  GFX::Buffer* pack1 = renderData.getBuffer("GMMStatisticsPack1");
  GFX::Texture* vpls = renderData.getTexture("VPLs");
  GFX::Texture* dbg = renderData.getTexture("DBG");
  updateBindings(context, {
    {"u_gmmStatisticsPack0", RHI::BindingResource{{pack0->buffer.get(), 0, pack0->buffer->size()}}},
    {"u_gmmStatisticsPack1", RHI::BindingResource{{pack1->buffer.get(), 0, pack1->buffer->size()}}},
    {"u_vpls", RHI::BindingResource{vpls->getUAV(0, 0, 1)}},
...
SSPGGMM_CopyPass::SSPGGMM_CopyPass() {
  auto [comp] = GFX::ShaderLoader_SLANG::load(
      "../Engine/Shaders/SRenderer/addon/pathguiding/sspg-gmm-copy.slang",
      std::array<std::pair<std::string, RHI::ShaderStages>, 1>{
          std::make_pair("ComputeMain", RHI::ShaderStages::COMPUTE),
      });
...
auto SSPGGMM_CopyPass::reflect() noexcept -> RDG::PassReflection {
  RDG::PassReflection reflector;
...
SSPGGMM_SamplePass::SSPGGMM_SamplePass() {
  auto [rgen] = GFX::ShaderLoader_SLANG::load<1u>(
      "../Engine/Shaders/SRenderer/addon/"
      "pathguiding/sspg-sgm-sample.slang",
      std::array<std::pair<std::string, RHI::ShaderStages>, 1>{
          std::make_pair("RgenMain", RHI::ShaderStages::RAYGEN),
      });
...
SSPGGMM_VisPass::SSPGGMM_VisPass() {
  auto [rgen] = GFX::ShaderLoader_SLANG::load<1u>(
      "../Engine/Shaders/SRenderer/addon/"
      "pathguiding/sspg-sgm-vis.slang",
      std::array<std::pair<std::string, RHI::ShaderStages>, 1>{
          std::make_pair("RgenMain", RHI::ShaderStages::RAYGEN),
      });
