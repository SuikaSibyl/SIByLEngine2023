...
// Source paper: "Hash Functions for GPU Rendering"
// @url: https://jcgt.org/published/0009/03/02/
// This is adapted from the Shadertoy implementation.
...
uint32_t mur(uint32_t a, uint32_t h) { a *= c1; a = rotr(a, 17u); a *= c2;
...
uint32_t city(uint2 s)
{
    uint32_t len = 8u;
    uint32_t a = len, b = len * 5u, c = 9u, d = b;
...
uint32_t city(uint4 s)
{
    uint32_t len = 16u;
    uint32_t a = bswap32(s.w);
    uint32_t b = bswap32(s.y);
    uint32_t c = bswap32(s.z);
    uint32_t d = bswap32(s.z);
    uint32_t e = bswap32(s.x);
    uint32_t f = bswap32(s.w);
    uint32_t h = len;
...
uint32_t jkiss32(uint2 p)
{
    ...
    uint32_t z = 345678912u, w = 456789123u, c = 0u;
    int t;
    y ^= (y << 5); y ^= (y >> 7); y ^= (y << 22);
...
/*
uint2 trig(uint2 p) {
    return uint2(float(0xffffff)*fract(43757.5453*sin(dot(vec2(p),vec2(12.9898,78.233)))));
}
*/
float trig(vec2 p)
{
