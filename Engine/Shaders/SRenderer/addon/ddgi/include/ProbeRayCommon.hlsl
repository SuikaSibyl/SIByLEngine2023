...
#include "../../../include/common/quaternion.hlsli"
...
void DDGIStoreProbeRayMiss(RWTexture2DArray<float4> RayData, uint3 coords, DDGIVolumeDescGPU volume, float3 radiance)
{
    ...
    {
        RayData[coords] = float4(asfloat(PACK_FLOAT3_UINT(radiance)), 1e27f, 0.f, 0.f);
    }
}

/**
 * Stores the provided radiance and/or hit distance in the volume's ray data texture array at the given coordinates.
 * Formats (compacts) the radiance value to the volume's ray data storage format.
 */
void DDGIStoreProbeRayFrontfaceHit(
    ...
    if (volume.probeRayDataFormat == RTXGI_DDGI_VOLUME_TEXTURE_FORMAT_F32x4) {
        ...
        if (maxComponent(radiance.rgb) <= c_threshold) radiance.rgb = float3(0.f, 0.f, 0.f);
        RayData[coords] = float4(asfloat(PACK_FLOAT3_UINT(radiance.rgb)), hitT, 0.f, 0.f);
    }
}
...
float3 DDGILoadProbeRayRadiance(RWTexture2DArray<float4> RayData, uint3 coords, DDGIVolumeDescGPU volume)
{
    ...
    {
        return UNPACK_FLOAT3_UINT(asuint(RayData[coords].r));
    }
