...
struct MLPHalf32X32<let N:int, Act:IActivationFn> : MLPHalf<N, 32> {
    ...
    [Differentiable]
    Output _foward(Input input, no_diff float grad_scalar = 1.0f) {
        HalfFeature<32> out_feature = input;
        [ForceUnroll] for (int i = 0; i < N; i++) {
        out_feature = LinearHalf32X32.eval(linears[i], out_feature, grad_scalar);
        [ForceUnroll] for (int j = 0; j < 32; j++)
        out_feature.vals[j] = Act.eval(out_feature.vals[j]); }
        return out_feature; }

    [Differentiable]
    static Output forward(no_diff MLPHalf32X32<N, Act> mlp, Input input, no_diff float grad_scalar = 1.0f) {
        return mlp._foward(input, grad_scalar); }
};
