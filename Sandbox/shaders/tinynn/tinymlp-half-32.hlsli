#ifndef _SRENDERER_TINYMLP_32_HEADER_
#define _SRENDERER_TINYMLP_32_HEADER_
...
struct Feature32_Half {
    float16_t vals[32];
};

struct Linear32_Half<let has_bias : bool> {
    typedef Feature32_Half Input;
    typedef Feature32_Half Output;
    ...
    __init(inout uint offset_prim,
        ...
        this.weights_view = TensorView_Half(offset_prim, offset_grad, 32, 32);
        offset_prim += 32 * 32; offset_grad += 32 * 32;
        if (has_bias) {
    ...
    Output forward(Input input, ThreadInfo thread_info) {
        ...
        Output out_feature;
        const SharedMemRef outPtr = calc_offset<32 * 32>(thread_info);
        load_output(outPtr, thread_info, out_feature);
        // output the result.
        return out_feature;
    }

    static Output forward(Linear32_Half<has_bias> layer, Input input, ThreadInfo thread_info) {
    ...
    void move_input_to_smem(
        ...
        SharedMemRef inPtr = calc_offset<32 * 32>(thread_info); ;
    ...
    void move_weights_to_smem<let colMajor : bool>(
        ...
        const SharedMemRef wtPtr = 4096 + calc_offset<32 * 32>(thread_info);
        const int threadIdInWarp = thread_info.thread_idx.x;
        const int warpId = thread_info.thread_idx.y;
    ...
    void load_output(SharedMemRef outPtr,
        ...
        if (has_bias) [ForceUnroll] for (int i = 0; i < 32; i++)
    ...
    void load_array<let colMajor : bool>(SharedMemRef memptr,
        ...
        if (!colMajor) { [ForceUnroll] for (int i = 0; i < 32; i++)
            // rowMajor matrix loading
            output.vals[i] = __inline_get_half_shared_buffer(
                memptr + threadIdInWarp * 32 + i);
        } else { [ForceUnroll] for (int i = 0; i < 32; i++)
    ...
    void store_array<let colMajor : bool>(
        ...
        if (!colMajor) { [ForceUnroll] for (int i = 0; i < 32; i++)
            ...
            (threadIdInWarp * 32 + i), float16_t(input.vals[i]));
        } else { [ForceUnroll] for (int i = 0; i < 32; i++)
            // rowMajor matrix writing
            __inline_set_half_shared_buffer(memptr +
    ...
    static uint calc_offset<let N : int>(ThreadInfo threadInfo) {
...
#endif // _SRENDERER_TINYMLP_32_HEADER_